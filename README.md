...
Through this project of building a personal finance tracker from scratch, we gained hands-on experience working with Rust and learned how to design and build a structured application using Rust's powerful ecosystem. We became familiar with some of the useful crate including actix_web for building a server, sqlx for database interactions, crossterm for writing text-based user interfaces and Ratatui for building aa efficient and user-friendly user interface. Additionally, we developed essential teamwork and problem-solving skills through collaboration and debugging together. Through group brainstorming we help each other with difficult issues we faced and make improvements to our project.
